...
def logistic(index,value,data,length,r0,C):
	random.seed(40)
		...
		for item in range(len(index)):
			...
			for i in range(len(index[item])):
					...
					if predicted*label>0:
						w[k]+= ((r*label*value[item][i]*math.exp(-(label*predicted)))/(1+math.exp(-(label*predicted)))) 
					else:
						w[k]+= (r*label*value[item][i])/(1+math.exp(label*predicted))
			b=b - 2*r/C * b
...
for r0 in [1,0.1,0.01,0.001,0.0001,0.00001]:
	for C in [0.1,1,10,100,1000,10000]:
		
		#print '========================================='
		#print 'Learning rate' , r0, 'Loss parameter', C
		#print '========================================='
		weight0,bias0=logistic(kfold1index,kfold1value,kfold1data,kfoldlength,r0,C)
		weight1,bias1=logistic(kfold2index,kfold2value,kfold2data,kfoldlength,r0,C)
		weight2,bias2=logistic(kfold3index,kfold3value,kfold3data,kfoldlength,r0,C)
		weight3,bias3=logistic(kfold4index,kfold4value,kfold4data,kfoldlength,r0,C)
		weight4,bias4=logistic(kfold0index,kfold0value,kfold0data,kfoldlength,r0,C)
...
#################train for 20 epochs######################################
def logistic_1(index,value,data,length,r0,C):
	global count20
	random.seed(40)
	w=np.random.uniform(-0.01,0.01,length+2)
...
weighttrain,biastrain=logistic_1(indextrain,valuetrain,datatrain,max(lengthtest,lengthtrain),best_r,best_C)
...
print ('Best value of test accuracy:',bestaccu,'%', 'across epoch:', bestepoch)
print ('Overall test set accuracy' , predictaccuval ,'%')
print ('Training accuracy:',bestaccu, '%')
